...
def combine_CAMEL_coefs(
    recipe, disc, eta, rho, CAMEL, coastal, AMEL, input_dir, fit=True
):

    print(f"Creating {recipe} {disc} for {CAMEL}...")
    os.makedirs(f"{input_dir}/{CAMEL}", exist_ok=True)
    ...
    for sector in [coastal, AMEL]:
        coefs[
            sector
        ] = f"{input_dir}/{sector}/{recipe}_{disc}_eta{eta}_rho{rho}_damage_function_coefficients.nc4"
    ...
    coefs["combined"].to_netcdf(
        f"{input_dir}/{CAMEL}/{recipe}_{disc}_eta{eta}_rho{rho}_damage_function_coefficients.nc4"
    )
    ...
    if fit == True:
        ...
        for sector in [coastal, AMEL]:
            fit[
                sector
            ] = f"{input_dir}/{sector}/{recipe}_{disc}_eta{eta}_rho{rho}_damage_function_fit.nc4"
        ...
        fit["combined"].to_netcdf(
            f"{input_dir}/{CAMEL}/{recipe}_{disc}_eta{eta}_rho{rho}_damage_function_fit.nc4"
        )
